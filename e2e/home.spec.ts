...
test.describe('Home page', () => {
  ...
  test('"New meeting" navigates to a valid meet-code URL', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/room\/[a-z0-9]{3}-[a-z0-9]{4}-[a-z0-9]{3}$/, { timeout: 5_000 })
  ...
  test('joining by code navigates to that room', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/room\/abc-defg-hij$/, { timeout: 5_000 })
  })

  test('pasting a full URL extracts just the path', async ({ page }) => {
    await page.getByPlaceholder(/code or link/i).fill('http://localhost:3000/room/abc-defg-hij')
    await page.getByRole('button', { name: /^join$/i }).click()
    await expect(page).toHaveURL(/\/room\/abc-defg-hij$/, { timeout: 5_000 })
  })
