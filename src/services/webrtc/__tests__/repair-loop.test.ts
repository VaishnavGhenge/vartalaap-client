...
describe('RepairLoop', () => {
    it('cancels destructive work on late success while retaining backoff until stable', () => {
        ...
        expect(attempts).toHaveLength(1)
        vi.advanceTimersByTime(750)
        expect(attempts.at(-1)?.attempt).toBe(2)
        loop.settle()
        vi.advanceTimersByTime(60_000)
        loop.schedule()
        vi.advanceTimersByTime(750)
        expect(attempts.at(-1)?.attempt).toBe(1)
    })
