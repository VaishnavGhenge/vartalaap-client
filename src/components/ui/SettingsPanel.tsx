...
function BackgroundEffectsGrid({ onChange }: { onChange: (pref: BackgroundEffectPreference) => void }) {
    ...
    const commit = (next: BackgroundEffectPreference) => {
        setPref(next);
        onChange(next);
    };
    ...
    const selectNone = () => commit({ mode: "off", imageDataUrl: pref.imageDataUrl });

    const selectBlur = (m: BackgroundEffectMode) => commit({ mode: m, imageDataUrl: pref.imageDataUrl });
...
export function SettingsPanel({ onClose, isVideoOff }: SettingsPanelProps) {
    ...
    return (
        <aside
            ...
            role="dialog"
            aria-label="Settings"
            className="call-stage fixed inset-y-0 right-0 z-[60] flex w-80 max-w-[90vw] flex-col border-l
        ...
        >
            ...
            <div className="flex flex-1 flex-col gap-7 overflow-y-auto px-5 py-6">
                ...
                <section className="flex flex-col gap-3">
                    ...
                    {isVideoOff ? (
                        <p
                            className="rounded-xl border border-[hsl(var(--border)/0.4)] bg-[hsl(var(--surface-2))]/60
                            px-4 py-3 text-sm text-[hsl(var(--muted-foreground))]"
                        >
                            Turn on your camera to use background effects.
                        </p>
                    ) : (
                        <BackgroundEffectsGrid
                            onChange={(pref) => {
                                void setBackgroundEffect(pref);
                            }}
                        />
                    )}
                </section>
